defmodule StarlingWeb.UserAuthTest do
  use StarlingWeb.ConnCase, async: true

  alias Starling.Accounts
  alias Starling.Accounts.Scope
  alias StarlingWeb.UserAuth

  import Starling.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, StarlingWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
